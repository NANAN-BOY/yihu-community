    ...
    <div v-loading="loading" class="project-list">
      <div v-for="(project, index) in filteredProjects" :key="index" class="project-card">
        ...
        <div class="project-description">
          {{ project.project_description.length > 20 ? project.project_description.slice(0, 20) + '...' : project.project_description }}
        </div>
        ...
        <div class="project-created-at">{{ project.formattedCreateAt }}</div>
      </div>
    </div>
  </template>
    ...
    try {
      ...
      if (data.success) {
        projectData.value = data.projects.map(project => ({
          ...project,
          project_name: project.projectDeclare_id,  // 可能需要映射实际字段
          project_description: project.projectDeclare_create_at,  // 可能需要映射实际字段
          formattedCreateAt: formatDate(project.projectDeclare_create_at),  // 格式化日期字段
          isOptimized: project.projectDeclare_draftEnable  // 可能需要映射实际字段
        }));
